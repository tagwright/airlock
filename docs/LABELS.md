...
- `airlock.<suffix>` is the primary, tool-branded form, and it leads every
  example in this doc.
- `tagwright.egress.<suffix>` is the org-namespaced alias.
...
| `airlock.mode` | enum | `alert` (or `AIRLOCK_DEFAULT_MODE`) | `audit`: evaluate the policy exactly as `alert` would, but route every would-be violation to the digest only, never an immediate alert. `alert`: evaluate and alert immediately on the first occurrence of a new violation identity, with windowed suppression of repeats. `block` is **reserved**, always a validation error in v1 (see Reserved surface below). |
...
| `airlock.policy` | csv of names | none | References one or more named policy sets from `airlock.yml`'s `policies:` map. Every referenced set's `allow`/`deny` union into this container's lists, and an unknown name is a validation error. See Policy sets below. |
...
| `airlock.deny` | csv of entries | none | Destinations that always violate, beating any allow. Deny has no `none` sentinel of its own, because an empty deny list is already its zero value. Combined with `airlock.allow: "*"`, this is the explicit denylist posture. |
...
- **IPv4/IPv6 literals and CIDRs** (`203.0.113.7`, `10.0.0.0/8`,
  ...
  directly, a hard match with no name evidence involved. An **IPv6 literal or
...
| Entry suffix `/udp` | Reserved for a future UDP-observing backend. Validation error in v1, since airlock evaluates TCP only. |
...
- `allow` and `deny` **union** across every referenced set plus the
  ...
  reference). Referencing two sets means both allowlists apply, with no
...
- Scalar fields (`mode`, `scope`, `alert.window`) resolve **label over
  ...
  scalar, with no label override to settle it, is a validation error. It
  is never silent last-one-wins.
...
- **List-valued fields** (`allow`, `deny`) **union across every applicable
  ...
  group. No precedence here: a second matching rule can only grant more,
  never silently remove an allowance another source granted.
- **Scalar fields** (`enable`, `mode`, `scope`, `alert.window`) resolve by
  ...
  1. The container's own labels (a policy set the label references travels
     at this same tier, since referencing a set is itself a per-container act).

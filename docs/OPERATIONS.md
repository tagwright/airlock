...
(an `allow: "*"` policy with no `deny` rules, which is a no-op policy that
can never fire, or a `@self`/`@project`/`net:<name>` entry used without
...
- `ig` needs `debugfs`/`tracefs` mounted and needs BTF (`/sys/kernel/btf/vmlinux`
  ...
  paper over. Most modern distro kernels ship it, but a stripped-down or
